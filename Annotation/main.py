...
class OVA_Labeling(QMainWindow):
    def __init__(self, args):
        ...
        self.radius = 1
        self.nth = 0
        self.model = Deeplab(class_num=2, output_stride=16,
        ...
        self.sum_frame_num_label.setText("/ " + str(len(self.args.image_path_list) - 1))
        self.total_frame_num = len(self.args.image_path_list) - 1
    ...
    def vis_(self):
        ...
        if len(mask_names) != len(self.args.image_path_list):
            prediction(self.pbar, self.args, nth, self.args.device)

        self.visualization_status = True
        self.basic_frame_block()

    def training(self):
        start_training(self.pbar, self.args, self.nth, self.args.device)
        self.nth += 1
        self.load_images()
    ...
    def load_images(self):
        ...
        if not os.path.exists(self.current_annotated_pkl):
            ...
            ova_model_num = len(self.args.class_names) - 1

            if self.nth == 0 and len(self.args.support_weighs_list) > 0:
                for weights_path in self.args.support_weighs_list:
                    print("Active Selection")
                    print(weights_path)
                    self.query_selector.active_(weights_path, self.pbar, self.current_annotated_pkl, self.model)
            else:
                for weights_path in glob(f"{self.args.dir_root}/{self.args.project_name}/{self.nth-1}_query/*.pt"):
                    print("Active Selection")
                    print(weights_path)
                    self.query_selector.active_(weights_path, self.pbar, self.current_annotated_pkl, self.model)
            for random_selection_num in range(ova_model_num - len(self.args.support_weighs_list)):
                print("Random Selection")
                self.query_selector.random_(self.pbar, self.current_annotated_pkl)
...
if __name__ == "__main__":
    ...
    from annotation_tool_parameters import Arguments
